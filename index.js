...
const amountRaised = GAMES_JSON.reduce((acc, curr) => acc + curr.pledged, 0)
...
gamesCard.innerHTML = `<p>🕹️ ${totalGames.toLocaleString('en-US')}</p>`
...
const unfundedGamesCount = GAMES_JSON.reduce(
  (acc, curr) => (curr.pledged < curr.goal ? acc + 1 : acc),
  0
)

// create a string that explains the number of unfunded games using the ternary operator
const description = `A total of $${amountRaised.toLocaleString(
  'en-US'
)} has been raised for ${totalGames}. Currently, ${unfundedGamesCount} ${
...
firstGameContainer.append(firstEl)

// do the same for the runner up item
const secondEl = document.createElement('p')
secondEl.innerHTML = second.name
secondGameContainer.append(secondEl)

// Customizations: Searching games
